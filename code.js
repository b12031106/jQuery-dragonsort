...
;(function($){
	...
	$.fn.dragonsort = function(_options, debug){
		...
		$(this).on('mousedown', '.dragonsort > '+options.childrenSelector, function(e){
			...
			//clone一份當helper
			$('body').append($(this).clone().addClass('helper').css({
				'position' : 'absolute',
				'margin' : 0
			...
			$('.helper').on('mousemove', function(e){
				...
				switch (options.dragDirection) {
					case 'vertical' :
						$('.helper').css({top : e.pageY-shiftY});
						break;
					case 'horizontal' :
						$('.helper').css({left : e.pageX-shiftX});
						break;
					case 'both':
					default :
						$('.helper').css({top : e.pageY-shiftY, left : e.pageX-shiftX});
						break;
				}	

				//取得helper中心點座標
				var helperCenterPosition = {
					'x' : $('.helper').position().left + $('.helper').width()/2,
					'y' : $('.helper').position().top + $('.helper').height()/2
				};
				...
				console.log('%s %s', targetElement.tagName, targetElement.className);
				$('.helper').show();
		...
		function findLastElement(_x, _y) {
			...
			do {
				...
				if (_target.tagName == 'HTML' || $(_target).hasClass('dragonsort') || $(prevElement).hasClass('dragonsort')) {
					break;
				}
